...
public class EntryPoint extends Application {
  ...
  public static void main(String[] args) {
    log.info("Starting.");
    objectJsonMapper.activateDefaultTyping(BasicPolymorphicTypeValidator.builder().build(),
  ...
  public void start(Stage primaryStage) {
    ...
    try {
      primaryStage.getIcons()
          .add(new Image(getClass().getResource("/icon.png").toURI().toURL().toString()));
    ...
    scene.getAccelerators().put(new KeyCodeCombination(KeyCode.L, KeyCombination.CONTROL_DOWN),
        () -> loadEvent.handle(null));
    scene.getAccelerators().put(new KeyCodeCombination(KeyCode.S, KeyCombination.CONTROL_DOWN),
        () -> saveEvent.handle(null));
    scene.getAccelerators().put(new KeyCharacterCombination("e"),
        () -> new TurnSkipClickedEvent(gameState).handle(null));
    scene.getAccelerators().put(new KeyCharacterCombination("s"),
        () -> new SkipSacrificeClickedEvent(gameState).handle(null));
