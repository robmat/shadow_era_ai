...
public class CardPainter {
  ...
  public GridPane paint(Card card, GridPane grid, int index, GameState gameState) {
    ...
    if (Objects.nonNull(card.getAttack(gameState))) {
      gridPane
          .add(new Label("ATK: " + card.getAttack(gameState) + " (" + card.getAttackType() + ")"),
              0, 1);
    }
    ...
    //row 4
    if ((gameState.cardIsCurrentHero(card) || gameState.cardIsInCurrentAllies(card)) && card
        .canAttack(gameState)) {
    ...
    if (!card.getAttachments().isEmpty()) {
      String attachments = card.getAttachments()
          ...
          .toString();
      gridPane
          .add(new Label("Attached: " + attachments.substring(1, attachments.length() - 1)), 0, 5,
              2, 1);
    }
    //end row 6
