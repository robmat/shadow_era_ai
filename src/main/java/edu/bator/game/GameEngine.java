...
public class GameEngine {
  ...
  private void expireEffects(GameState gameState) {
    List<Card> cards = Stream.concat(gameState.yourHeroAlliesAndSupportCards().stream(),
        gameState.enemyHeroAlliesAndSupportCards().stream()).collect(
        Collectors.toList());
    cards.stream()
        .filter(card -> !card.getEffects().isEmpty())
        .forEach(card -> {
          ...
          log.debug(
              format("Turn %s phase %s", gameState.getCurrentTurn(), gameState.getGamePhase()));
  ...
  private void cardDied(Card card, GameState gameState, LinkedList<Card> diedFrom,
      GraveyardLinkedList graveyard) {
