...
public class GameState {
  ...
  public void init() {
    ...
    enemyHand.add(allCardsSet.cloneByName("Bazaar", Owner.ENEMY));
    yourHand.add(allCardsSet.cloneByName("Bazaar", Owner.YOU));
  ...
  public List<Card> allCardsInPlay() {
    Set<Attachment> attachments = Stream
        .concat(enemyHeroAlliesAndSupportCards().stream(), yourHeroAlliesAndSupportCards().stream())
        .map(Card::getAttachments)
        .reduce((a, b) -> new HashSet<>() {{
          addAll(a);
          addAll(b);
        }}).orElse(new HashSet<>());
    List<Card> cardList = Stream.concat(Stream.concat(enemyHeroAlliesAndSupportCards().stream(),
        yourHeroAlliesAndSupportCards().stream()), attachments.stream())
        .distinct()
  ...
  public Card enemyHeroBasedOnPhase() {
    return GamePhase.YOU_ACTION.equals(getGamePhase()) ? enemyHero
        : (GamePhase.ENEMY_ACTION.equals(getGamePhase())) ? yourHero : null;
  }
