...
public class GoodAscendant extends Artifact {
  ...
  public void gamePhaseChangeEvent(GameState gameState) {
    ...
    if (GamePhase.YOU_PREPARE.equals(gameState.getGamePhase()) && gameState.getYourSupport()
        .contains(this)) {
      gameState.getYourAllies().forEach(ally -> {
        HealingUtil.heal(2, ally);
      });
    }
    if (GamePhase.ENEMY_PREPARE.equals(gameState.getGamePhase()) && gameState.getEnemySupport()
        .contains(this)) {
  ...
  public boolean ableToApplyAbilityTo(Card card, GameState gameState) {
    boolean you = gameState.yourAction() &&
        ...
        gameState.getEnemySupport().stream()
            .anyMatch(c -> Objects.equals(c.getName(), "Evil Ascendant"));

    boolean enemy = gameState.enemyAction() &&
        gameState.getEnemySupport().contains(this) &&
        gameState.getYourSupport().stream()
            .anyMatch(c -> Objects.equals(c.getName(), "Evil Ascendant"));
  ...
  @Override
  public void applyAbility(Card target,
      GameState gameState) { //TODO not tested, no evil ascendant yet
