...
public class ReinforcedArmor extends Attachment {
    ...
    public boolean ableToApplyAbilityTo(Card card, GameState gameState) {
        boolean you = gameState.getYourHand().contains(this) && gameState.getYourAllies().contains(card) && gameState.yourAction();
        boolean enemy = gameState.getEnemyHand().contains(this) && gameState.getEnemyAllies().contains(card) && gameState.enemyAction();
