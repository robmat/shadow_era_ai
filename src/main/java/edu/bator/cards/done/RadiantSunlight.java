...
public class RadiantSunlight extends Ability {
    ...
    public boolean ableToApplyAbilityTo(Card card, GameState gameState) {
        boolean you = gameState.yourAction() && gameState.getEnemyHero().equals(card) && gameState.getYourHand().contains(this);
        boolean enemy = gameState.enemyAction() && gameState.getYourHero().equals(card) && gameState.getEnemyHand().contains(this);
