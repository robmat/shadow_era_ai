...
public class ShrineofNegatia extends Ability {
    ...
    public void wasCasted(GameState gameState) {
        ...
        CardUiHelper.showDialog((stage, gridPane) -> {
            ...
            gameState.allCardsInPlay()
                    ...
                    .forEach(card -> {
                        EventHandler<MouseEvent> onMouseClicked = new ShrineofNegatia.ShrineofNegatiaTargetClickedEvent(gameState, card, stage);
    ...
    public boolean ableToApplyAbilityTo(Card target, GameState gs) {
        Stream<Card> enemyCards = Stream.concat(Stream.of(gs.getEnemyHero().getWeapon(), gs.getEnemyHero().getArmor()), gs.getEnemySupport().stream());
        Stream<Card> yourCards = Stream.concat(Stream.of(gs.getYourHero().getWeapon(), gs.getYourHero().getArmor()), gs.getYourSupport().stream());
