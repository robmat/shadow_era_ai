...
public class SmashingBlow extends Ability {
    ...
    public void determineCastable(GameState gameState) {
        ...
        if (isCastable()) {
            boolean enemy = gameState.enemyAction() && ( nonNull(gameState.getYourHero().getWeapon()) || nonNull(gameState.getYourHero().getArmor()));
            boolean you = gameState.yourAction() && ( nonNull(gameState.getEnemyHero().getWeapon()) || nonNull(gameState.getEnemyHero().getArmor()));
    ...
    public void wasCasted(GameState gameState) {
        ...
        BiConsumer<Stage, GridPane> consumer = (stage, gridPane) -> {
            ...
            if (gameState.yourAction()) {
                cardStream = Stream.of(gameState.getEnemyHero().getWeapon(), gameState.getEnemyHero().getArmor())
                        .filter(Objects::nonNull);
            }
            if (gameState.enemyAction()) {
                cardStream = Stream.of(gameState.getYourHero().getWeapon(), gameState.getYourHero().getArmor())
                        .filter(Objects::nonNull);
            }
            cardStream
                    .forEach(card -> {
    ...
    private class SmashingBlowClickedEvent implements EventHandler<MouseEvent> {
        ...
        SmashingBlowClickedEvent(GameState gameState, Card target, Stage stage, SmashingBlow smashingBlow) {
